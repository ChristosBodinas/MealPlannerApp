...
public interface ExerciseRepository extends JpaRepository<Exercise, Long> {
    ...
    @Query("SELECT x FROM Exercise x LEFT JOIN FETCH x.levels l " +
    ...
    @Query("SELECT x FROM Exercise x WHERE x.user.id = :userId " +
            "AND (:text IS NULL OR LOWER(x.name) LIKE LOWER(CONCAT('%', :text, '%')))")
